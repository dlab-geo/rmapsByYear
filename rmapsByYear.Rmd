...
my_rmaps_directory <- "~/Documents/Dlab/dlab_workshops/rmapsByYear"
...
#### Challenge
```
Create an ichoropleth map of a subset of the years of data, from 200
...
## Sharing your rMap
http://rpubs.com/pfrontiera/226917
```


## More customizations

The ichropleth function is a great convenience wrapper around Datamaps. It doesn't try to do too much. It does one thing very well - creating an annual map series for state-level data. To do more, you can write create maps directly with Datamaps. Code examples for this are shown in [this blog post](http://rmaps.github.io/blog/posts/animated-choropleths/).


```{r, eval=FALSE}
ic3 <- function(x, data, pal = "Blues", nodata_color="white", nodata_label="NA", ncuts = 5, animate = NULL, play = F, map = 'usa', legend = TRUE, labels = TRUE, include_lowest=FALSE, my_breaks = NULL, my_title='', ...){
...
```{r, eval=FALSE}
map6 <- ic3(vals ~ State, data = indata, animate="year", my_title='doh')
map6$set(
  geographyConfig = list(
    popupTemplate = "#! function(geo, data){ return '<div class=\"hoverinfo\">' 
      + geo.properties.name + ' ' + data.year +  '<br>' +  + data.vals + '%' +  
