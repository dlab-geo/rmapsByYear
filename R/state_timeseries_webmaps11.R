...
map = readOGR("state_squares_from_kwalker.json", "usa") # using local squares map
map_names <- data.frame(id=0:(length(map@data$id)-1),
                       State=map$id )
map_df <- fortify(map)
...
gg <- gg + geom_text(data=centers, aes(label=State, x=x, y=y), size=1)
