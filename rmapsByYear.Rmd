...
Another option is to extend the code for ichoropleth to make it a bit more customizable. We did this by creating the function `ichoropleth2` which is in the R script file `ichoropleth2.R`. To access this function source the R script.
...
Let's try mapping no data values with the `ichoropleth2` function.
...
As you can see, that's some pretty wide data with one column for 87 years of data (1917 - 2013). Moreover, because the column names are numbers the are read in with an "X" prepended.
